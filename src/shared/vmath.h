#ifndef TWBL_SRC_SHARED_VMATH_H
#define TWBL_SRC_SHARED_VMATH_H

#if __has_include(<base/vmath.h>)
...
template<typename T>
constexpr inline bool closest_point_on_line(vector2_base<T> LinePointA, vector2_base<T> LinePointB, vector2_base<T> TargetPoint, vector2_base<T> &OutPos)
{
	vector2_base<T> AB = LinePointB - LinePointA;
	T SquaredMagnitudeAB = dot(AB, AB);
	if(SquaredMagnitudeAB > 0)
	{
		vector2_base<T> AP = TargetPoint - LinePointA;
		T APdotAB = dot(AP, AB);
		T t = APdotAB / SquaredMagnitudeAB;
		OutPos = LinePointA + AB * clamp(t, (T)0, (T)1);
		return true;
	}
	else
		return false;
